...
onChangef(e){
	...
	onConfirm(event) {
		...
		this.setData({ show: false,zhihangname:value});
	  },
	onChanget(event) {
	    const { picker, value, index } = event.detail;
	    // Toast(`当前值：${value}, 当前索引：${index}`);
		...
		onConfirmt(event) {
		    ...
		    // Toast(`当前值：${value}, 当前索引：${index}`);
			console.log(`当前值：${value}, 当前索引：${index}`)
			this.setData({ showt: false,teamname:value});
		  },
	  onCancel() {
	    Toast('取消');
	  },
...
//
  ...
  submit: function() {
	...
	var zhihangname=this.data.zhihangname
...
userInfo_nickname:app.globalData.userInfo.nickName,
    ...
    util.wxRequest(url, params, data => {
		console.log(data)
      if (data.code == 200) {
		  app.globalData.userInfo=data.newsuser
        wx.showToast({
          title: data.msg,
          icon: 'none',
		  duration:3000,
